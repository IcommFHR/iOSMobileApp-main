...
export class LoginPage {
  ...
  pinverify() {
        ...
        this.authservice.MobilePinNumberValidate(this.validatepindata)
          .then(data => {
            ...
            if (this.res.d.ErrFlag == 0) {
              this.hideLoading();
              if (sessionStorage.getItem('userUnregisterUser') != null && sessionStorage.getItem('userUnregisterUser') == '1' && sessionStorage.getItem('userUnregisterUser') != '') {
                this.showUnRegisterAlert("App Unregister", "Are you sure want remove your account register?", 1);
              } else {
                sessionStorage.setItem('ispinavailable', '1');
                sessionStorage.setItem('mPin', this.pin);
                this.navCtrl.push(HomePage, { "JsonData": this.res.d });
   ...
   //Showing alert popup
   showUnRegisterAlert(title: string, subtitle: string, alertcode: any) {
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: data => {
            ...
            let mob: string = String(data.Mobile);
            let isMobileNo: boolean = mob.match(/^[0-9]+$/) ? true : false;
            console.log('isMObiileNO', isMobileNo)
            if (data.Mobile == null || data.Mobile == '') {
              this.showToast("Please enter Mobile Number");
              return false;
            }else if (!isMobileNo) {
              this.showToast("Please Enter a valid Mobile No");
            }else if (mob.length != 10) {
              this.showToast("Mobile number should be matched with your registeration");
            } else {
              this.showLoading();
              ...
              if (this.network.type == 'none') {
                ...
                this.authservice.forgotpin(forgotpindata)
                  .then(data => {
                    this.res = data;
                    this.hideLoading();
                    if (this.res.d.ErrFlag == 0) {
